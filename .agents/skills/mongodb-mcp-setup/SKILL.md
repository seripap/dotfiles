...
- **If no `CODEX_*` variables are present** → the user is running a **shell-based client** (Codex, Cursor, Gemini CLI, Copilot CLI, etc.). Credentials are configured via shell profile environment variables.
